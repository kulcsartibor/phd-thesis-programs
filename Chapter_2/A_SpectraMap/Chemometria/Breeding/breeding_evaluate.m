...
% 	[nbh param1 param2] = agr_opres(['(' fs1{1} ')'],['(' fs2{1} ')'], X , DistM);
% 	[nbh param1 param2] = agr_opres(fs1{1},fs2{1}, X , DistM);
	[nbh param1 param2 k1 k2] = agr_opres(fs1,fs2, X , DistM);
	
	%Chrom
	popu.unit{j}.tree = tree1; %write back the tree
	popu.unit{j}.tree = tree2;
	%   popu.chrom{j}.mse = mse;
	%   popu1.chrom{j}.fitness = Mt * Mc;
	%   popu2.chrom{j}.fitness = Mt * Mc;
	popu.unit{j}.fitness = nbh;
% 	disp(nbh);
