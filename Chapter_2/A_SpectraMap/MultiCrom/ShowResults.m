...
[~,terms] = breeding_result(popu,1);

disp(terms{1});
disp(terms{2});

MultiChromMap = [eval(terms{1}) eval(terms{2})];
...
[Mt,Mc,~,~] = trustcont(10, X, MdsMap, DistM, squareform(pdist(MdsMap))); %#ok<*NASGU,*ASGLU>
fitness(1) = Mt*Mc;

SmMap = sammon(X,2,1000,'steps',[],DistM);
[Mt,Mc,V,U] = trustcont(10, X, SmMap, DistM, squareform(pdist(SmMap)));
fitness(2) = Mt*Mc;
