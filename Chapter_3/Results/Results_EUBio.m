...
Index = intersect(find(Y(:,30) ~= -9999), find(Y(:,15) ~= -9999));
Y = Y(Index,[15 30]);
X = X(Index,:);
...
Pindex = [1 2]; % {'D5 [°C]' 'Density [kg/dm3]'}
...
vist = PrintTable;
vist.Format = 'tex';
vist.addRow('Method','$M_{t}$','$M_{c}$','Metric Error','Rate of false neighbors (CFPP0)', 'Rate of false neighbors (Density)')
reft = PrintTable;
reft.Format = 'tex';
reft.addRow('Regression Method', 'Correlation Coefficent for CFPP0', 'Correlation Coefficent for Density')
advt = PrintTable;
advt.Format = 'tex';
advt.addRow('Regression Method', 'Correlation Coefficent for CFPP0', 'Correlation Coefficent for Density')

metr = PrintTable;
metr.Format = 'tex';
metr.addRow('Method','Accuracy', 'Precision','Recall','F1 score')
...
figure(11) % subplot(2,3,2)
plot(NormMDSP(:,1), NormMDSP(:,2), 'r.');
...
[Prec Rec F1 Acc] = Precision(DistM, MdsDist);
metr.addRow('MDS', num2str(Acc), num2str(Prec), num2str(Rec), num2str(F1))
...
figure(12) % subplot(2,3,3)
plot(NormPLS1(:,1), NormPLS1(:,2), 'r.');
axis([-0.1 1.1 -0.1 1.1])
...
[Prec Rec F1 Acc] = Precision(DistM, PLSDist1);
metr.addRow('PLS CFPP', num2str(Acc), num2str(Prec), num2str(Rec), num2str(F1))
...
figure(14) % subplot(2,3,4)
plot(NormPLS2(:,1), NormPLS2(:,2), 'r.');
axis([-0.1 1.1 -0.1 1.1])
title('PLS 2D Visualization (CFPP0)', 'FontSize', 12, 'FontWeight','bold')
xlabel('Dim 1')
ylabel('Dim 2')
text(0,-0.3,{['Mt: ' num2str(Mt)]; ['Mc: ' num2str(Mc)]; ['Err: ' num2str(Err)]}, 'FontWeight','bold')
vist.addRow('2D PLS (D5°C)', num2str(Mt), num2str(Mc), num2str(Err),...
	num2str(fnn(NormPLS2,Y(:, Pindex(1)))),num2str(fnn(NormPLS2,Y(:, Pindex(2)))));
[Prec Rec F1 Acc] = Precision(DistM, PLSDist2);
metr.addRow('PLS Dens', num2str(Acc), num2str(Prec), num2str(Rec), num2str(F1))
...
figure(15) % subplot(2,3,5)
plot(NormPCA(:, 1), NormPCA(:, 2), 'r.');
axis([-0.1 1.1 -0.1 1.1])
...
%%
[Prec Rec F1 Acc] = Precision(DistM, PCADist);
%%
metr.addRow('PCA', num2str(Acc), num2str(Prec), num2str(Rec), num2str(F1))
%% Regression section
% =============================================================================================
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% =============================================================================================
...
DelDist(:,:,4) = PLSDist2;
DelDist(:,:,5) = PCADist;
Methods = {'Agregates'; 'MDS mapping'; 'PLS CFPP0'; 'PLS Dens'; 'PCA Mapping'};
...
vist.saveToFile('.\TexTables\EUBiovisual.tex');
reft.saveToFile('.\TexTables\EUBioRegr.tex');
advt.saveToFile('.\TexTables\EUBiodAdv.tex');
metr.saveToFile('.\TexTables\EUBioMetr.tex');
