...
for op=1:maxord; % Calc. of the theoretical bacg.matrices of RSM unsing dif. pol. orders
    ...
    for i=1:r;
        for j=1:c;
              OrdSel(op).CBKG(i,j)=polyval(Order(op).MPC(j,:),RSMmat(i,c1)./RSMmat(i,c2),[],Order(op).MU(:,j));  
        end
    end
end
...
for i=1:Sr
    for j=1:c
      cSM(i,j)=polyval(Order(1,(ordopt(j,1))).MPC(j,:),SMmat(i,c1)./SMmat(i,c2),[],Order(ordopt(j,1)).MU(:,j)); 
    end
end
