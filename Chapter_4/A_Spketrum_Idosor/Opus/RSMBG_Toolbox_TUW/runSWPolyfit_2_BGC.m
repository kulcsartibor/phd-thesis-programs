...
for op=1:maxord; % Calc. of the polynomial fittings
    for j=1:c;
        [P,S,MU]=polyfit(Xref,RSMmat(:,j),op);
        Order(op).MPC(j,:)=P;
        Order(op).MU(:,j)=MU;
    end
end
...
disp ('Step 2...')
for op=1:maxord; % Calc. of the theoretical bacg.matrices of RSM unsing dif. pol. order
  OrdSel(op).CBKG=ones(r,c);
    for i=1:r; % Spectra in RSMmat
        for j=1:c; %Columns in RSMmat
           OrdSel(op).CBKG(i,j)=polyval(Order(op).MPC(j,:),RSMmat(i,c1),[],Order(op).MU(:,j));
        end
    end
end
...
for i=1:Sr
    for j=1:c
    cSM(i,j)=polyval(Order(1,(ordopt(j,1))).MPC(j,:),SMmat(i,c1),[],Order(ordopt(j,1)).MU(:,j)); 
    end
end
