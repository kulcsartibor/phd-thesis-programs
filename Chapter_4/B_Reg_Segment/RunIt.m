...
theta = X\Y;
Yg = X*theta;
figure('Name','Globális modell','NumberTitle','off','Color',[1 1 1])

plot(Y,Yg,'+','MarkerSize',1)
...
hline = zeros(3,1);
for i = 1:3
	hline(i) = line([0 1],[i*sgm 1+i*sgm],'LineStyle','--','LineWidth',2,'Color',color(i,:));
...
title('$Global~model$','Interpreter','latex','FontSize',12)
legend(hline,'1\sigma','2\sigma','3\sigma','Location','SouthEast')
axis square
...
for i = 1:numel(segment)-1
	rectangle('Position',[segment(i).lx+20, 0, segment(i).rx-segment(i).lx-40, segment(i).mc/mmc],'FaceColor',[0 0.75 1]);
end
...
%%
subplot(5,1,5)
plot(cumsum(Y-Ym(10:end)),'k','LineWidth',2)
