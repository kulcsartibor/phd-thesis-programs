...
% plot(node(:,1),node(:,2),'rd');
 	...
 	load vpHs
 	load vF
   node=[node; [(interp1(vpHs,vF,[7.25:1:10.25]))' [7:10]']];
end
...
%fact=(ndata+gamma)/(ndata-gamma); %FPE
fact=(ndata+(log(ndata)-1)*gamma)/(ndata-gamma); %FPE


GCV(nadd)=fact*data(:,3)'*P*P*data(:,3)/ndata
