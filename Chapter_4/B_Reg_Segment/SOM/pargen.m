...
for j=1:nstab
   ...
   plot([node(tri(i,1),1) node(tri(i,3),1)], [node(tri(i,1),2) node(tri(i,3),2)],'k-')
   plot([node(tri(i,2),1) node(tri(i,3),1)], [node(tri(i,2),2) node(tri(i,3),2)],'k-')
   %fill([node(tri(i,1),1) node(tri(i,2),1) node(tri(i,3),1) ],[node(tri(i,1),2) node(tri(i,2),2) node(tri(i,3),2) ],'g','linewidth',1.4)
...
for j=1:nstab
   ...
   bs=param(i,2)/(1-param(i,1));
   cs=param(i,3)/(1-param(i,1));
   ...
   if (0<alfa1)&(alfa1<1)&(0<alfa2)&(alfa2<1)
      ...
      plot([cord1(1) cord2(1) ], [cord1(2) cord2(2) ],'k--','linewidth',1.5);
   end
...
plot(vF(1:end-10),vpHs(1:end-10),'k','linewidth',2);
 xlabel('F_{NaOH}(k)');
 ylabel('pH(k)');
